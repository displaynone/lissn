...
import com.displaynone.lissn.SortBy
...
private fun createSelectionString(input: AssetsOptions): String {
    ...
    input.album?.let {
        selectionBuilder.append("${MediaStore.Audio.Media.ALBUM} = '${input.album}'") // Use album name instead of ID
    }
...
fun parseSortByKey(key: String): String =
    SortBy.fromKeyName(key)?.mediaColumnName ?: run {
